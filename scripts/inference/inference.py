...
def plot_combined_images(model, prompt, num_inference_steps=100):
    print("Loading model")
    ...
    image_finetuned = pipeline(prompt=prompt, num_inference_steps=num_inference_steps).images[0]
    ...
    image_vanilla = pipeline(prompt=prompt, num_inference_steps=num_inference_steps).images[0]
    ...
    filename = f"img_{current_datetime.strftime('%H:%M:%S %d/%m/%y')}"
